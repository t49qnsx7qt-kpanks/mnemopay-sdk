/**
 * MnemoPay SDK — 1,000,000 Transaction Stress Test
 *
 * 100 agents × 10,000 ops = 1,000,000 total transactions.
 *
 * Same harness as 300K / 500K — pure scale-up to prove invariants
 * (ledger balance, replay-detection, p99 latency) at 1M ops.
 *
 * SLOs (must pass):
 *   - totalOps        >= 1,000,000
 ...
 *   - p99 latency     < 500 ms
 *
 * Test runtime budget: 30 minutes.
 */
...
const OPS_PER_AGENT = 10_000;
const TOTAL_OPS_TARGET = AGENT_COUNT * OPS_PER_AGENT; // 1,000,000
...
describe("MnemoPay SDK — 1M stress", () => {
  it(
    ...
    async () => {
      ...
      console.log("\n" + banner);
      console.log("  MNEMOPAY SDK — 1,000,000 TRANSACTION STRESS TEST");
      console.log(banner);
