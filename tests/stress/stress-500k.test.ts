/**
 * MnemoPay SDK — 500K Transaction Stress Test
 *
 * 100 agents × 5,000 ops = 500,000 total transactions.
 *
 * Same op mix and adversarial injection as the 300K harness — this is a
 * pure scale-up to prove the SDK holds its invariants (ledger balance,
 * replay-detection, p99 latency) at half a million concurrent operations.
 *
 * SLOs (must pass):
 *   - totalOps        >= 500,000
 ...
 *   - p99 latency     < 500 ms
 *
 * Test runtime budget: 15 minutes.
 */
...
const OPS_PER_AGENT = 5000;
const TOTAL_OPS_TARGET = AGENT_COUNT * OPS_PER_AGENT; // 500,000
...
describe("MnemoPay SDK — 500K stress", () => {
  it(
    ...
    async () => {
      ...
      console.log("\n" + banner);
      console.log("  MNEMOPAY SDK — 500K TRANSACTION STRESS TEST");
      console.log(banner);
